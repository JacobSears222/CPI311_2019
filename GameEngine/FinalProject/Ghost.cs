...
namespace CPI311.GameEngine
{
    public class Ghost : GameObject
    {
        ...
        public Ghost(TerrainRenderer terrain, ContentManager Content, Camera camera, GraphicsDevice graphicsDevice, Light light, Random random) : base()
        {
            Terrain = terrain;
            ...
            Texture2D texture = Content.Load<Texture2D>("Blinky");
            Renderer renderer = new Renderer(Content.Load<Model>("Sphere"), Transform, camera, Content, graphicsDevice, light, 1, "SimpleShading", 1f, texture);
        ...
        //Player Collision
        public virtual bool CheckCollision(Pacman pacman)
        {
            Vector3 normal;
            if(pacman.Get<Collider>().Collides(this.Get<Collider>(), out normal))
            {
                path = null;
                return true;
            }
